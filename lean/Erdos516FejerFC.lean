...
/-!
# Erdős 516: an explicit counterexample to the Fejér-gap variant

This file imports the registered Formal Conjectures target and fills its
`answer` with `False`. The construction is explicit and kernel-checked.
-/
...
theorem exponents_hasFejerGaps : HasFejerGaps exponents := by
...
theorem ratio_on_interval (p : ℕ) {u : ℝ}
    (hlo : s (p + 2) ≤ u) (hhi : u ≤ s (p + 3)) :
    Erdos516.ratio (Real.exp u) f ≤ 1 / 2 := by
...
theorem ratio_eventually_le_half :
    ∀ᶠ r in atTop, Erdos516.ratio r f ≤ (1 / 2 : ℝ) := by
...
theorem counterexample_limsup_ne_one : limsup (fun r => Erdos516.ratio r f) atTop ≠ 1 :=
...
theorem explicit_counterexample :
    Differentiable ℂ f ∧ HasFejerGaps exponents ∧
      (∀ k, coefficients k ≠ 0) ∧
...
/-- The answer to the registered Fejér-gap variant of Erdős 516 is `False`. -/
...
#print axioms erdos_516_fejer_gap_variant_solved
